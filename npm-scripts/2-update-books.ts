...
async function get_tutorials(docs) {
  try {
    ...
    if (config.ok) {
      ...
      return { myst: true, book: {url: docs, title, source, description: ''}, tutorials: list };
    }
  } catch {}
...
export async function main(catalog) {
  ...
  for (const distro of distros) {
    ...
    if (myst) {
      ...
      books.distros.push({book: {...book, title: 'Docs for ' + distro.title}, distro: distro.name})
    }
  ...
  for (const plugin of plugins) {
    ...
    if (!plugin.docs || seenBook.has(plugin.docs)) { continue; }

    const { myst, book, tutorials} = await get_tutorials(plugin.docs)
    console.log(book, tutorials)
    if (myst) {
