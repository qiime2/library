...
export function applyFilters () {
    ...
    for (const repo_overview of overview_store.repo_overviews) {
        if (String(repo_overview["Repo Name" as keyof Object]).startsWith(overview_store.search_filter)) {
            filtered_overviews.push(repo_overview);
        }
    }

    for (const distro of overview_store.filter_distros) {
        filtered_overviews = filtered_overviews.filter(e => e["Distros"].includes(distro))
    }

    for (const epoch of overview_store.filter_epochs) {
        filtered_overviews = filtered_overviews.filter(e => e["Epochs"].includes(epoch))
    }
