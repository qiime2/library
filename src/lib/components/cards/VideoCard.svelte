...
<div class='w-80 flex flex-col my-4'>
    <a class='relative hover:drop-shadow-md rounded-2xl border border-gray-200' href={`https://www.youtube.com/watch?v=${video.id}`} title={video.title}>
        <img class='rounded-2xl object-cover object-center h-52' width=480 height=360 src={`https://i.ytimg.com/vi_webp/${video.id}/hqdefault.webp`} alt={video.title}/>
        <div class='w-full h-full z-10 absolute left-0 top-0 opacity-0 hover:opacity-100 transition-opacity'>
