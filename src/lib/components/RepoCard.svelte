...
<div class="prose max-w-none repo-card rounded-2xl mb-2">
    <h1>
        <a href="/plugin/{repo_overview.owner}/{repo_overview.name}">
            {repo_overview.owner}/{repo_overview.name}
        </a>
    </h1>
    <p class="description">
    ...
    <div class="container2">
        <div>
            ...
            <a href={repo_overview.docs}>{repo_overview.docs}</a>
        </div>
        <div style="word-break: break-all"><span class="font-bold">Distros:</span><CutOffList list={repo_overview.distros} collapseNumber={5} /></div>
    </div>
    <div class="container">
        ...
        <div>
            ...
            {formatDate(repo_overview.last_commit.date)}&nbsp;
        </div>
        <div>
            <span class="font-bold">Build Status:</span>
            <span class="{getStatusColor()} h-fit">
                {repo_overview.last_commit.status}
            </span>
        </div>
    </div>
</div>
